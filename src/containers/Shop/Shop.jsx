...
const Shop = () => {
  ...
  const getResource = async (url) => {
    ...
    const menuListMc = res.restorans.McDonalds.map(
      ({ id, name, img, price }) => {
        return {
          id,
          name,
    ...
    <>
      <div className={styles.container}>
        <div className={styles.container__restoran}>
          ...
          {restorans && (
            <ul>
              {restorans.map((element) => (
                <button
                  className={
                    curentRestorans === element
                      ? styles.restoran__button_active
                      : styles.restoran__button
                  }
                  key={element}
                  onClick={() => setCurrentRestorans(element)}
                >
                  {element}
                </button>
              ))}
        ...
        <div className={styles.container__menu}>
          {menuKFC && curentRestorans === "KFC" && (
            <MenuList restoran={menuKFC} />
          )}

          {menuRis && curentRestorans === "Ris" && (
            <MenuList restoran={menuRis} />
          )}

          {menuMc && curentRestorans === "McDonalds" && (
            <MenuList restoran={menuMc} />
          )}
        </div>
      </div>
    </>
